...
class MD_functions:
    ...
    def initialize_trajectory(self,parser, input_args=None):
        ...
        parser.add_argument('-sub_unit_top','--sub_unit_topology_files',help='All topology files for sub-units. Used when superposing sub-units (optional).',nargs='+',default='');
    ...
    def hydrophobic_Cb_coordination_number(self,traj, topology, plotter):
        ...
        CB = topology.select("protein and name CB and (resname ALA or resname VAL or resname LEU or resname ILE or resname PRO or resname PHE or resname MET or resname TRP)");
        CB_pairs = np.array(
    ...
    def DSSP_structure(self, traj, plotter, doPlotting=False, saveDeltaDSSP=False):
        ...
        if saveDeltaDSSP:
            ...
            np.savetxt(self.save_folder + 'dDSSP_holo_strand_' + self.file_name_end + '.txt', dssp_holo_stran -dssp_strand);
            ...
            np.savetxt(self.save_folder + 'dDSSP_apo_strand_' + self.file_name_end + '.txt', dssp_apo_stran -dssp_strand);
    ...
    def computeFrameToFrameCalphaContacts(self, traj, choice='protein', async=True, computeContactNorm=None, process_count=multiprocessing.cpu_count()):
        if computeContactNorm is None:
            computeContactNorm = _computeContactNorm
        atom_indices = traj.topology.select(choice);
        traj.atom_slice(atom_indices, inplace=True);
        ...
        for i in range(0,nFrames):
            ...
            for j in range(i+1,nFrames):
                ...
                tasks.append((tmpMap1,tmpMap2,i,j))
        if async: 
            ...
            results = [0 for i in range(len(tasks))]
            for i, t in enumerate(tasks):
                if i % 5000 == 0:
                    print("Computing frame to frame distance %s/%s. %s percent"%(i,len(tasks), (100.0*i)/len(tasks)))
                results[i] = computeContactNorm(*t)
        idx = 0
        for i in range(0,nFrames):
            #build a list of tasks
            for j in range(i+1,nFrames):
        ...
        return distances, distanceMatrices
    ...
    def computeCentroidDistance(self, traj, residues1, residues2, plotter, file_tag='', showPlots=False):
        ...
        np.savetxt(self.save_folder + 'distances_between_residues_'+ file_tag + self.file_name_end + '.txt', distances);
        return;
    ...
    def computeProbeDistances(self, traj, probe_residues1, probe_residues2, plotter, showPlots=False):
        ...
        atom_indices1 = traj.topology.select('protein and residue '+ str(probe_residues1[0]) + ' and sidechain and !(type H)');
        atom_indices2 = traj.topology.select('protein and residue '+ str(probe_residues1[1]) + ' and sidechain and !(type H)');
        ...
        atom_indices1 = traj.topology.select('protein and residue '+ str(probe_residues2[0]) + ' and sidechain and !(type H)');
        atom_indices2 = traj.topology.select('protein and residue '+ str(probe_residues2[1]) + ' and sidechain and !(type H)');
    ...
    def computeHelixDirection(self, traj, part, plotter, show_plots=False,save_data=True):
        ...
        if show_plots:
            ...
            plotter.show();
        if save_data:
            np.savetxt(self.save_folder + 'helix_direction_' + part + '_' + self.file_name_end + '.txt',helix_direction);
...
anglesNative_psi[0,j]))/nAngles + 0.25*(1.0 + math.cos(angles_phi[i,j]-anglesNative_phi[0,j]))/nAngles;
    ...
    def correlationToNativeDihedral(self, traj, plotter, atom_sel = 'all', domain_name='', nativeHolo=True):
        ...
        if len(atom_indices) > 0:
            ...
            if nativeHolo:
                ...
                np.savetxt(self.save_folder + 'holo_' + domain_name + '_dihedral_angle_correlation_' + self.file_name_end + '.txt',correlation);
            else:
    ...
    def correlationNeighborDihedral(self, traj, plotter):
        ...
        np.savetxt(self.save_folder + 'dihedral_angle_neighbor_correlation_' + self.file_name_end + '.txt',correlation);
    ...
    def radiusOfGyration(self, traj, plotter, atom_sel = 'all', domain_name = ''):
        ...
        if domain_name != '':
            np.savetxt(self.save_folder + domain_name + '_radius_gyration_' + self.file_name_end + '.txt',radiusOfGyration);
        else:
    ...
    def DRID(self, traj, topology, plotter, native = ""):
        ...
        for i in range(0,nFrames):
            DRID_distance[i] = 1/(3.0)*np.sum((drid_vectors[i,::3]-drid_native[0,::3])**2 + (drid_vectors[i,1::3]-drid_native[0, 1::3])**2 + (drid_vectors[i,2::3]-drid_native[0,2::3])**2);
    ...
    def compute_energy_profile(self, data, title, normalizationPoint, plotter):
        ...
        np.savetxt(self.save_folder + "energy_" + title + "_" + self.file_name_end + ".txt", energy-normalizationValue);
...
def _computeContactNorm(tmpMap1, tmpMap2,i,j):
