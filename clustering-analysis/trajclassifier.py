...
def to_matrix_indices(vector_idx, rowcount):
    ...
    TODO use squareform som scikit
    """
    lastcount = 0
...
def transform_and_scale(frame_distances):
    ...
    scaler = MinMaxScaler() #Can also use MinMaxScaler() or StandardScaler()
...
def create_classifier_cvs(clustering_id, training_samples, target_values, scaler, classifier, trainingstep, query="name CA", cvs=None):
...
class CAContactsVectorizer(object):
    ...
    def process(self, traj):
        dists = MD_fun.MD_functions().getAllCalphaDistances(traj, query=self.query)
        return vectorize_matrices(dists)
